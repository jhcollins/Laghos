...
void MarkingLS::createSDF(ParMesh *mesh,
  ...
  std::cout<<nodes_cnt<<std::endl;
  for (int i = 0; i < nodes_cnt; i++)
    {
      //mfem::Vector v;
      //v=(vxyz(i),vxyz(i+nodes_cnt));
      //std::cout<<vxyz(i)<< " "<<vxyz(i+nodes_cnt)<<" "<<sdf(i)<<std::endl;
      
      sdf(i) = func({vxyz(i),vxyz(i+nodes_cnt)});
      //std::cout<<vxyz(i)<<", "<<vxyz(i+nodes_cnt)<<std::endl;
    }
  sdf.ExchangeFaceNbrData();
}
...
void MarkingLS::tagCells(ParMesh *mesh,
  ...
  for (int i=0; i<NE; ++i){
    ...
    elementtag.ParFESpace()->GetElementDofs(i,edofs);
	...
	for (long unsigned int k=0; k<edofs.Size(); ++k){
	  elementtag(edofs[k])=count;
	 }
  ...
  elementtag.ExchangeFaceNbrData();
}
